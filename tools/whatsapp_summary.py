...
def get_chat_history(chat_name_query: str, timeframe_hours: int = 24, search_text: str = None) -> str:
    ...
    try:
        ...
        if not messages:
            ...
            return (
                f"Chat found: '{group_name}', but no messages{search_info} "
...
def _resolve_chat(cursor, chat_name_query: str):
    ...
    cursor.execute(
        "SELECT DISTINCT group_id, group_name FROM whatsapp_messages WHERE group_name LIKE ?",
        (f"%{chat_name_query}%",)
    )
...
def _list_active_chats(cursor) -> str:
    cursor.execute(
        """SELECT group_name, group_id, COUNT(*) as msg_count, MAX(timestamp) as last_msg
           FROM whatsapp_messages
