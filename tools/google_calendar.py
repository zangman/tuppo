    ...
    if not creds:
      if not os.path.exists(CREDENTIALS_FILE):
        raise FileNotFoundError(
          f"credentials.json not found at {CREDENTIALS_FILE}. Please download it from Google Cloud Console.")
    ...
    for cal_id in calendars_to_check:
      try:
        events_result = service.events().list(
          calendarId=cal_id, timeMin=time_min, timeMax=time_max, singleEvents=True, orderBy='startTime').execute()
...
def propose_calendar_event(summary: str = None,
                           start_iso: str = None,
                           end_iso: str = None,
                           description: str = "",
                           requester_id: str = "Unknown") -> str:
  """
  ...
  try:
    ...
    try:
      ...
      cursor = conn.cursor()
      cursor.execute("SELECT display_name FROM contacts WHERE chat_id = ? OR chat_id = ?",
                     [requester_id, requester_id.replace('@c.us', '').replace('@lid', '')])
      row = cursor.fetchone()
      conn.close()
      if row:
        display_name = row[0]
    ...
    text = (f"<b>🔔 EVENT PROPOSAL RECEIVED</b>\n\n"
    ...
    keyboard = {
      "inline_keyboard": [[{
        "text": "Approve ✅",
        "callback_data": f"app_{proposal_id}"
      }, {
        "text": "Reject ❌",
        "callback_data": f"rej_{proposal_id}"
      }]]
    }

    url = f"https://api.telegram.org/bot{bot_token}/sendMessage"
    payload = {"chat_id": owner_id, "text": text, "parse_mode": "HTML", "reply_markup": json.dumps(keyboard)}
...
def list_calendar_events(time_min=None, time_max=None) -> str:
  ...
  try:
    ...
    for cid in calendars_to_query:
      try:
        events_result = service.events().list(
          calendarId=cid, timeMin=time_min, timeMax=time_max, singleEvents=True, orderBy='startTime').execute()
...
def create_calendar_event(calendar_id='primary',
                          summary: str = None,
                          start_iso: str = None,
                          end_iso: str = None,
                          description: str = "") -> str:
...
def update_calendar_event(calendar_id='primary',
                          event_id: str = None,
                          summary: str = None,
                          start_iso: str = None,
                          end_iso: str = None,
