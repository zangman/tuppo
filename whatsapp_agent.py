...
async def whatsapp_webhook(request: Request):
  ...
  try:
    response_text, pp, tp = await core_brain.get_llm_response(
      session_id, user_input, system_prompt_override=system_prompt)
  except Exception as e:
  ...
  if response_text.strip() == 'TAKE_MESSAGE':
    ...
    async def delayed_ack():
      ...
      try:
        requests.post(WHATSAPP_API_URL, json={"chatId": chat_id, "text": f"🤖 {ack}", "mark_unread": True})
  ...
  async def delayed_send():
    await asyncio.sleep(delay)
    try:
      requests.post(WHATSAPP_API_URL, json={"chatId": chat_id, "text": f"🤖 {response_text}", "mark_unread": True})
